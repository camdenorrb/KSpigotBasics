...
/**
 * Version independent sounds
 *
 * @param aliases The aliases for the sound
 */
enum class Sound(vararg aliases: String) {
	...
	/**
	 * A Bukkit instance of the sound
	 */
	val bukkitSound by lazy {

		aliases.forEach {
			return@lazy bukkitSoundMap[it] ?: return@forEach
		}

		error("Can't find valid Bukkit sound for $name")
	}
	...
	companion object {
		...
		/**
		 * A map containing all the Bukkit sounds
		 * Name --> Sound
		 */
		val bukkitSoundMap by lazy {
			BukkitSound.values().associateBy { it.name }
		}
